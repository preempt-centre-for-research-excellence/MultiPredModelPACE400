...
calibration_plot <- function(time.to.pace, gaf, caarms.cd, caarms.tc, caarms.pa, pre.cond, pc1.thick, pc2.thick, t){
  ...
  cal.plot <- ggplot(cal_plot_data, aes(x = x, y = Value, group = Type)) + 
         ...
         scale_x_continuous(name =paste("Predicted ", as.character(t),"-year Probability of Transition to Psychosis",sep = ""), breaks = c(0.2,0.4,0.6,0.8), limits=c(0.15, 0.9)) +
         scale_y_continuous(name=paste("Observed ", as.character(t),"-year Probability of Transition to Psychosis",sep = ""), breaks = c(0.2,0.4,0.6,0.8), limits=c(0.15, 0.9)) +
         theme_bw() +
         labs(color = '') +
...
bl.multi <- cph(S ~ time.to.pace + gaf + pre.cond + pc1.surf + pc2.surf, x=TRUE, y=TRUE, surv = TRUE, time.inc = 10*12)
cox.res <- cox_model(bl.multi, 3, cox.res[[1]], cox.res[[2]], bl.conf, LR.orig)

bl.multi <- cph(S ~ time.to.pace + gaf + pre.cond + pc1.curve + pc2.curve, x=TRUE, y=TRUE, surv = TRUE, time.inc = 10*12)
cox.res <- cox_model(bl.multi, 4, cox.res[[1]], cox.res[[2]], bl.conf, LR.orig)

bl.multi <- cph(S ~ time.to.pace + gaf + pre.cond + pc1.vol + pc2.vol, x=TRUE, y=TRUE, surv = TRUE, time.inc = 10*12)
...
year2_plot <- calibration_plot(time.to.pace, gaf, caarms.cd, caarms.tc, caarms.pa, pre.cond, pc1.thick, pc2.thick, 2)
year5_plot <- calibration_plot(time.to.pace, gaf, caarms.cd, caarms.tc, caarms.pa, pre.cond, pc1.thick, pc2.thick, 5)
year10_plot <- calibration_plot(time.to.pace, gaf, caarms.cd, caarms.tc, caarms.pa, pre.cond, pc1.thick, pc2.thick, 10)
...
pre.cond <- relevel(factor(pre.cond),ref=3)
# Preperation for cox model fit
dd <- datadist(time.to.pace, gaf, ttmt, pre.cond, cog1, cog2, caarms.cd, caarms.tc)
options(datadist='dd')
units(dtime) <- 'Month'
